...
find_node_path() {
  if [[ -n "$NODE_PATH_BIN" ]] && [[ -x "$NODE_PATH_BIN" ]]; then
    return 0
  fi
...
install_plugin() {
  ...
  if [[ -f "$oc_config" ]]; then
    saved_plugin_config=$(INSTALLER_CONFIG_FILE="$oc_config" node -e "
      ...
      if (entry || config?.plugins?.slots?.memory === 'claude-mem' || allowHasClaudeMem) {
        ...
        // Also remove any stale memory-slot reference — if the slot points to a plugin
        // that isn't in entries, OpenClaw's config validator rejects ALL commands.
        // claude-mem should no longer claim the exclusive memory slot by default.
  ...
  # This write is guaranteed even when the config file is first materialized later.
  if [[ -f "$oc_config" ]]; then
    ...
    # Config doesn't exist yet; a later OpenClaw command may materialize it.
...
###############################################################################
# OpenClaw plugin entry configuration
# Ensures plugins.entries["claude-mem"] exists and is enabled without
# claiming the exclusive plugins.slots.memory slot.
###############################################################################

configure_plugin_entry() {
  ...
  if [[ ! -f "$config_file" ]]; then
    # No config file exists — create one with an enabled plugin entry only
    info "Creating OpenClaw configuration with claude-mem plugin entry..."
    ...
    "
    success "Created ${config_file} with claude-mem plugin entry enabled"
    return 0
  fi

  # Config file exists — update it to ensure the plugin entry is enabled
  info "Updating OpenClaw configuration to enable claude-mem plugin entry..."
  ...
  INSTALLER_CONFIG_FILE="$config_file" node -e "
    ...
    // Remove stale memory slot claim. claude-mem should coexist with memory-core.
    if (config.plugins.slots.memory === 'claude-mem') {
      delete config.plugins.slots.memory;
    }
  ...
  success "claude-mem plugin entry enabled in ${config_file}"
}
...
"
    ...
    error "Neither python3 nor node is available to write observation feed config"
    return 1
  fi
...
main() {
  ...
  # --- Step 4: OpenClaw plugin entry configuration ---
  echo ""
  info "${COLOR_BOLD}[4/8]${COLOR_RESET} Configuring OpenClaw plugin entry..."
  configure_plugin_entry
