...
describe("Observation I/O event handlers", () => {
  ...
  it("session_start tracks session without sending init to worker", async () => {
    ...
    assert.ok(!initRequest, "should defer init until before_agent_start");
    assert.ok(logs.some((l) => l.includes("Session tracking initialized")));
    assert.ok(logs.some((l) => l.includes("openclaw-agent-1-")));
  });

  it("session_start followed by before_agent_start sends exactly one init", async () => {
    const { api, fireEvent } = createMockApi({ workerPort });
    claudeMemPlugin(api);

    await fireEvent("session_start", { sessionId: "test-session-1" }, { sessionKey: "agent-1" });
    ...
    assert.equal(initRequests.length, 1, "should init only once after tracking + before_agent_start");
  });

  it("after_compaction preserves session tracking without re-init", async () => {
    const { api, fireEvent } = createMockApi({ workerPort });
    claudeMemPlugin(api);
    ...
    const initRequests = receivedRequests.filter((r) => r.url === "/api/sessions/init");
    assert.equal(initRequests.length, 0, "should not re-init after compaction");
  });
  ...
  it("tool_result_persist sends observation to worker", async () => {
    ...
    // Track + init session so observation privacy checks can pass in the real worker
    await fireEvent("session_start", { sessionId: "s1" }, { sessionKey: "test-agent" });
    await fireEvent("before_agent_start", { prompt: "help me" }, { sessionKey: "test-agent" });
    await new Promise((resolve) => setTimeout(resolve, 100));
    ...
    }, { sessionKey: "test-agent", workspaceDir: "/workspace/test-agent" });
  ...
  it("tool_result_persist truncates long responses", async () => {
    ...
    await fireEvent("before_agent_start", { prompt: "hello" }, { sessionKey: "truncate-test" });
    await fireEvent("tool_result_persist", {
      toolName: "Bash",
      params: { command: "ls" },
      message: {
    ...
    }, { sessionKey: "truncate-test", workspaceDir: "/workspace/truncate-test" });
  ...
  it("agent_end sends summarize and complete to worker", async () => {
    ...
    // Establish tracked + initialized session
  ...
  it("agent_end extracts text from array content", async () => {
    ...
    await fireEvent("before_agent_start", { prompt: "help me" }, { sessionKey: "array-content" });
    await new Promise((resolve) => setTimeout(resolve, 100));
  ...
  it("uses custom project name from config", async () => {
    ...
    await fireEvent("session_start", { sessionId: "s1" }, { sessionKey: "custom-project" });
    await fireEvent("before_agent_start", { prompt: "hello" }, { sessionKey: "custom-project" });
    await new Promise((resolve) => setTimeout(resolve, 100));
  ...
  it("reuses same contentSessionId for same sessionKey", async () => {
    ...
    }, { sessionKey: "reuse-test", workspaceDir: "/workspace/reuse-test" });
