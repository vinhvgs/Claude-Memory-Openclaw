...
# Tests the OpenClaw gateway detection, plugin install, and plugin entry config.
...
###############################################################################
# Test: configure_plugin_entry() — creates new config
###############################################################################

echo ""
echo "=== configure_plugin_entry() ==="
...
test_configure_new_config() {
  ...
  configure_plugin_entry >/dev/null 2>&1
...
# Test: configure_plugin_entry() — updates existing config
test_configure_existing_config() {
  ...
  # Verify stale memory slot was cleared
  local memory_slot
  memory_slot="$(node -e "const c = JSON.parse(require('fs').readFileSync('${config_file}','utf8')); console.log(c.plugins.slots.memory || '');")"
  assert_eq "memory-core" "$memory_slot" "Existing memory slot preserved when owned by memory-core"
...
# Test: configure_plugin_entry() — preserves existing claude-mem config
...
for fn in find_openclaw check_openclaw install_plugin configure_plugin_entry; do
